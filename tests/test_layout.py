...
def test_k_grouped_psum_sf_layout_kernels() -> None:
    ...
    for mn, real_ks_cpu, aligned_ks_cpu, psum_layout, num_groups, gran_k, k_alignment in enumerate_k_grouped_psum_sf_layout():
        ...
        packed_sf = get_k_grouped_mn_major_tma_aligned_packed_ue8m0_tensor(fp32_sf, grouped_layout, aligned_ks_cpu, gran_k, k_alignment, use_psum_layout=True, use_padded_sf_layout=True)
        ...
        unsynced_packed_sf = get_k_grouped_mn_major_tma_aligned_packed_ue8m0_tensor(
            fp32_sf, grouped_layout, None, gran_k, k_alignment, use_psum_layout=True, use_padded_sf_layout=True)
        ...
        empty_ks_packed_sf = get_k_grouped_mn_major_tma_aligned_packed_ue8m0_tensor(
            fp32_sf, grouped_layout, [], gran_k, k_alignment, use_psum_layout=True, use_padded_sf_layout=True)
        assert torch.equal(empty_ks_packed_sf[:sum(packed_sf_ks)], ref_packed_sf)

        # Performance
        t = bench_kineto(lambda: get_k_grouped_mn_major_tma_aligned_packed_ue8m0_tensor(fp32_sf, grouped_layout, aligned_ks_cpu, gran_k, k_alignment, use_psum_layout=True, use_padded_sf_layout=True), 'pack_fp32_into_ue8m0')
