...
static void k_grouped_fp8_gemm_tn_contiguous(const std::pair<torch::Tensor, torch::Tensor>& a,
                                             ...
                                             const std::string& compiled_dims,
                                             const bool& use_psum_layout,
                                             const bool& use_padded_sf_layout) {
    // Must be 1D1D kernel
    DG_HOST_ASSERT(std::get<0>(recipe) == 1 and std::get<1>(recipe) == 1);

    // `k_alignment` must be a multiple of the FP8 UMMA K step (32)
    ...
    const auto arch_major = jit->device.get_arch_major();
    if (arch_major == 10) {
        const auto sfa = layout::transform_k_grouped_sf_into_required_layout(a.second, ks_cpu, grouped_layout, recipe, k_alignment, use_psum_layout, use_padded_sf_layout);
        const auto sfb = layout::transform_k_grouped_sf_into_required_layout(b.second, ks_cpu, grouped_layout, recipe, k_alignment, use_psum_layout, use_padded_sf_layout);
        sm100_k_grouped_fp8_gemm_1d1d(a.first, sfa, b.first, sfb, c, d, m, n, grouped_layout, gran_k, k_alignment,
                                       cute::UMMA::Major::MN, cute::UMMA::Major::MN, compiled_dims, use_psum_layout, use_padded_sf_layout);
    } else if (arch_major == 12) {
...
static void k_grouped_fp8_gemm_nt_contiguous(const std::pair<torch::Tensor, torch::Tensor>& a,
    ...
    const auto sfa = layout::transform_k_grouped_sf_into_required_layout(a.second, ks_cpu, grouped_layout, recipe, 128, false, use_padded_sf_layout);
    const auto sfb = layout::transform_k_grouped_sf_into_required_layout(b.second, ks_cpu, grouped_layout, recipe, 128, false, use_padded_sf_layout);
...
static void register_apis(pybind11::module_& m) {
    ...
    m.def("k_grouped_fp8_gemm_tn_contiguous", &k_grouped_fp8_gemm_tn_contiguous,
          ...
          py::arg("compiled_dims") = "mn",
          py::arg("use_psum_layout") = false,
          py::arg("use_padded_sf_layout") = false);
