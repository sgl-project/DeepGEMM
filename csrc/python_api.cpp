...
void m_grouped_bf16_gemm_nn_contiguous_wrapper(const torch::Tensor& a, const torch::Tensor& b, const torch::Tensor& d, const torch::Tensor& m_indices, const std::string& compiled_dims) {
    deep_gemm::gemm::m_grouped_bf16_gemm_nn_contiguous(a, b, d, m_indices, compiled_dims);
}
...
void k_grouped_bf16_gemm_tn_contiguous_wrapper(const torch::Tensor& a, const torch::Tensor& b, const torch::Tensor& d, c10::List<int64_t> ks, const torch::Tensor& ks_tensor, const c10::optional<torch::Tensor>& c, const std::string& compiled_dims) {
    std::vector<int> ks_vec;
    ks_vec.reserve(ks.size());
    for(const auto i : ks) {
        ks_vec.push_back(i);
    }
    deep_gemm::gemm::k_grouped_bf16_gemm_tn_contiguous(a, b, d, ks_vec, ks_tensor, c, compiled_dims);
}
...
torch::Tensor fp8_mqa_logits_wrapper(const torch::Tensor& q, const torch::Tensor& k, const torch::Tensor& v, const torch::Tensor& weight, const torch::Tensor& cu_seq_len_k_start, const torch::Tensor& cu_seq_len_k_end, bool clean_logits, const int64_t max_seqlen_k) {
    return deep_gemm::attention::fp8_mqa_logits(q, {k, v}, weight, cu_seq_len_k_start, cu_seq_len_k_end, clean_logits, max_seqlen_k);
}
...
TORCH_LIBRARY(deep_gemm, m) {
    ...
    m.def(R"(m_grouped_bf16_gemm_nn_contiguous(Tensor a, Tensor b, Tensor d, Tensor m_indices, str compiled_dims="") -> ())");
    m.impl("m_grouped_bf16_gemm_nn_contiguous", torch::kCUDA, [](const torch::Tensor& a, const torch::Tensor& b,
                                                                  const torch::Tensor& d, const torch::Tensor& m_indices,
                                                                  const std::string& compiled_dims) {
        deep_gemm_wrappers::m_grouped_bf16_gemm_nn_contiguous_wrapper(a, b, d, m_indices, compiled_dims);
    });
    ...
    m.def(R"(k_grouped_bf16_gemm_tn_contiguous(Tensor a, Tensor b, Tensor d, int[] ks, Tensor ks_tensor, Tensor? c=None, str compiled_dims="") -> ())");
    m.impl("k_grouped_bf16_gemm_tn_contiguous", torch::kCUDA, [](const torch::Tensor& a, const torch::Tensor& b, const torch::Tensor& d,
                                                                 at::IntArrayRef ks,
                                                                 const torch::Tensor& ks_tensor,
                                                                 const c10::optional<torch::Tensor>& c,
                                                                 const std::string& compiled_dims) {
        std::vector<int64_t> ks64(ks.begin(), ks.end());
        c10::List<int64_t> ks_list(ks64);
        deep_gemm_wrappers::k_grouped_bf16_gemm_tn_contiguous_wrapper(a, b, d, ks_list, ks_tensor, c, compiled_dims);
    });
    ...
    m.def(R"(fp8_mqa_logits(Tensor q, Any kv, Tensor weights, Tensor cu_seq_len_k_start, Tensor cu_seq_len_k_end, bool clean_logits=True, int max_seqlen_k=0) -> Tensor)");
    m.impl("fp8_mqa_logits", torch::kCUDA, [](
        ...
        return deep_gemm_wrappers::fp8_mqa_logits_wrapper(q, k, v, weights, cu_seq_len_k_start, cu_seq_len_k_end, clean_logits, max_seqlen_k);
    });
    ...
    m.impl("einsum", torch::kCUDA, [](const std::string& expr, const torch::Tensor& a, const torch::Tensor& b, const torch::Tensor& d, const c10::optional<torch::Tensor>& c, bool use_cublaslt=false) {
