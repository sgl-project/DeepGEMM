...
static std::tuple<int, int, int, int, int> get_block_config_for_mega_moe(
    ...
    auto [cluster_size, block_m, store_block_m, block_k, num_epilogue_warpgroups] = [&]() -> std::tuple<int, int, int, int, int> {
        ...
        if (num_expected_tokens_per_expert <= 8.5) {
            ...
            // Under kind::mxf4, smem_a_per_stage = load_block_m * block_k / 2 must be a multiple of the
            // 1024-byte smem alignment; bump block_m (→ load_block_m=16) for the MXF4 path only.
...
static std::pair<int, int> get_pipeline_config_for_mega_moe(
    ...
    const int smem_a_size_per_stage = use_mxf4_kind ? (load_block_m * block_k / 2)
                                                     : (load_block_m * block_k);
    const int smem_b_size_per_stage = use_mxf4_kind ? (block_n * block_k / 2)
                                                     : (block_n * block_k);
