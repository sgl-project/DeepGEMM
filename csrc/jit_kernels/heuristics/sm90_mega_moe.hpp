...
// L2-lag schedule constants (schedule selection: `get_sm90_buffer_schedule` below)
...
// Schedule a symm buffer is sized for. Decided once here from the wave-size knob (Python `num_experts_per_wave`, 0 when unset) and
// the capacity, returned to Python with the ring capacity and the encoded lag and passed back at every launch, so the ring capacity
// and the kernel schedule can never disagree.
//   knob 0 (unset): kLag from kSm90AutoLagMinTokens tokens per rank (a lag-sized ring running the L2-lag schedule), kFullPool below it
//   knob -1:        kWave, ring sized by the SM100 occupancy heuristic at the capacity
//   knob N > 0:     kWave, ring sized for a fixed wave of N experts (at call time the wave size is re-derived from the stored capacity, like SM100)
enum class SM90BufferSchedule { kFullPool, kWave, kLag };

static SM90BufferSchedule get_sm90_buffer_schedule(const int& num_max_tokens_per_rank, const int& num_experts_per_wave_knob) {
    DG_HOST_ASSERT(num_experts_per_wave_knob >= -1);
    if (num_experts_per_wave_knob != 0)
        return SM90BufferSchedule::kWave;
    return num_max_tokens_per_rank >= kSm90AutoLagMinTokens ? SM90BufferSchedule::kLag : SM90BufferSchedule::kFullPool;
}
...
// Lag schedule: the ring holds the lag + group units plus the in-flight blocks (get_sm90_lag_ring_blocks), sized at BLOCK_M 128.
static std::pair<int, int> get_sm90_lag_ring_tokens(
    const int& num_ranks, const int& num_max_tokens_per_rank, const int& num_topk, const int& num_experts_per_rank) {
    const int lag = get_sm90_auto_late_lag(num_max_tokens_per_rank, num_topk, num_experts_per_rank);
    ...
    return {std::min(math::align(tokens, layout::kSM90LCMBlockM), num_max_pool_tokens), lag_encoded};
}

// Wave schedule: the ring capacity is derived from the wave size, mirroring SM100's causality
// (E_wave is decided first, the pool capacity is derived from it) instead of
// deriving E_wave from a user-supplied capacity. -1 = auto wave size via the SM100 occupancy heuristic,
// N > 0 = fixed wave size for the derivation only.
static int get_sm90_wave_ring_tokens(
    const int& num_ranks, const int& num_experts, const int& num_experts_per_rank,
    ...
    return std::min(math::align(get_num_wave_pool_tokens_for_mega_moe_sm90(
                    ...
                    layout::kSM90LCMBlockM),
                    num_max_pool_tokens);
}

// Ring capacity (0 = full-pool buffers, no ring) and encoded L2 lag (0 = wave schedule) of a symm buffer.
static std::pair<int, int> get_num_ring_tokens_for_sm90_mega_moe(
    const int& num_ranks, const int& num_experts, const int& num_experts_per_rank,
    const int& num_max_tokens_per_rank, const int& num_topk,
    const int& hidden, const int& intermediate_hidden,
    const int& num_experts_per_wave_knob, const int& l2_act_sf_gran_k) {
    const auto schedule = get_sm90_buffer_schedule(num_max_tokens_per_rank, num_experts_per_wave_knob);
    if (schedule == SM90BufferSchedule::kFullPool)
        return {0, 0};
    DG_HOST_ASSERT(num_max_tokens_per_rank % layout::kSM90LCMBlockM == 0 and
                   "num_max_tokens_per_rank must be token-aligned before deriving a ring capacity");
    if (schedule == SM90BufferSchedule::kLag)
        return get_sm90_lag_ring_tokens(num_ranks, num_max_tokens_per_rank, num_topk, num_experts_per_rank);
    return {get_sm90_wave_ring_tokens(
                num_ranks, num_experts, num_experts_per_rank,
                num_max_tokens_per_rank, num_topk, hidden, intermediate_hidden,
                num_experts_per_wave_knob, l2_act_sf_gran_k),
            0};
}
...
static MegaMoESM90Config get_mega_moe_config_sm90(
    ...
    const int num_wave_bound_tokens_per_rank =
        std::min(num_max_tokens_per_rank, align(std::max(call_bound, 1), layout::kSM90LCMBlockM));
    ...
    if (num_ring_tokens != 0) {
        DG_HOST_ASSERT(num_ring_tokens % layout::kSM90LCMBlockM == 0);
