...
static void sm100_bf16_mega_gate(const torch::Tensor& x, const torch::Tensor& weight,
    ...
    // Avoid spills in the six-warpgroup routing-control epilogue.
    deep_jit::cuda::CompilerOptions compiler_options;
    if (config.num_gate_warpgroups == 6 &&
...
static void __instantiate_kernel() {{
    ...
    >);
        ...
        (fix_routing_mask ? fix_routing_mask->data_ptr() : nullptr) != nullptr,
        (force_random ? force_random->data_ptr() : nullptr) != nullptr), compiler_options);
