// F4: Dedicated FP4×FP4 GEMM kernel using PACKED FP4 SMEM + tcgen05.mma.kind::mxf4
//
// This kernel is a clone of sm100_fp8_fp4_gemm_1d1d.cuh, modified to:
//   1. Use cutlass::float_e2m1_t (PACKED variant: 2 elements/byte) for both A and B
//      operands instead of float_e2m1_unpacksmem_t (1 element/byte).
//   2. Double UMMA_K from 32 → 64 elements per instruction (mxf4 block_scale.block32).
//   3. Halve A/B SMEM bytes per stage (packed FP4 stores 2 elements per byte).
//   4. Emit tcgen05.mma.cta_group::{1,2}.kind::mxf4.block_scale via
//      ptx::SM100_MMA_MXFP4_SS / SM100_MMA_MXFP4_2x1SM_SS (F2 wrappers).
//   5. Build the SMEM descriptors with manual *packed-byte* strides, bypassing
//      mma::sm100::make_umma_desc/advance_umma_desc_lo whose sizeof(dtype_t)=1 byte
//      assumption would over-count K-strides by 2x for packed FP4.
//   6. Issue TMA loads via cute::SM90_TMA_LOAD_2D::copy directly so that the
//      per-row swizzle atom matches the packed-FP4 TMA descriptor (smem_inner_dim
//      = swizzle_mode*2 packed elements).
//
// Invariants set by the JIT runtime:
//   - BLOCK_K   : 256 elements (== 128 packed bytes == swizzle_a_mode == 128)
//   - UMMA_K    : 64 elements per mxf4 instruction
//   - kSwizzle* : 128B for A/B
//   - A and B are K-major (the host dispatch enforces this for fp4_requires_k_major)
//   - gran_k_a == gran_k_b == 32 (UE8M0 VS=32 → 8 SFs per BLOCK_K=256)
//
// SF organisation: with VS=32 and BLOCK_K=256 elements, one BLOCK_K consumes 8
// SF entries per MN row, packed into 2 uint32 words (4 SFs each). The kernel
// keeps kNumSFAStagesPerLoad=1, but uses sfa_id = k * 2 (and sfb_id = k * 2) so
// that consecutive UMMA_K=64 instructions step through 2 SFs each.
...
// Helper: 2D TMA load for packed-FP4 operands. Issues a single SM90_TMA_LOAD_2D
// per (inner, outer) box so the swizzle atom (smem_inner_dim) matches the packed
// FP4 tensor-map (one atom == swizzle_mode * 2 packed elements per row).
//
// We always use the 1-CTA SM90_TMA_LOAD_2D form because — like the existing
// mxf8f6f4 GEMM kernel — the TMA-load warp runs on every CTA in the cluster
// (warp 0 + elect_one_sync), each CTA issues its OWN slice load. The cluster
// arrival counts (`empty_barriers[i]->init(1)` etc.) already assume per-CTA
// per-iteration arrives; switching to `SM100_TMA_2SM_LOAD_2D` would double-count
// transaction bytes onto the leader barrier and deadlock the consumer wait.
CUTLASS_DEVICE void packed_fp4_tma_load_2d(
    void const* desc_ptr,
    cutlass::arch::ClusterTransactionBarrier* barrier_ptr,
    uint8_t* smem_ptr_bytes,
    const uint32_t& inner_idx, const uint32_t& outer_idx) {
    cute::SM90_TMA_LOAD_2D::copy(
        desc_ptr, reinterpret_cast<uint64_t*>(barrier_ptr),
        static_cast<uint64_t>(cute::TMA::CacheHintSm100::EVICT_NORMAL),
        smem_ptr_bytes, inner_idx, outer_idx);
}
...
sm100_fp4_fp4_gemm_1d1d_impl(int* grouped_layout,
                             uint32_t shape_m, uint32_t shape_n, uint32_t shape_k,
    ...
    // PACKED FP4 dtypes: stored 2 elements per byte
    ...
    // MMA Configs (mxf4: K=64 per instruction, doubled vs mxf8f6f4)
    ...
    // BLOCK_K is in ELEMENTS. For packed FP4 the per-row byte width is BLOCK_K / 2.
    // Constraint: the InstrDescriptorBlockScaled `a_sf_id_` / `b_sf_id_` fields
    // are only 2 bits wide (values 0..3). With mxf4 each instruction consumes
    // an SF *pair* (sfa_id steps by 2), so BLOCK_K can hold at most 2 pairs
    // (= 4 SFs) per K block, i.e. BLOCK_K <= 4 * VS = 4 * 32 = 128 elements.
    DG_STATIC_ASSERT(BLOCK_K == 128, "Packed FP4 path expects BLOCK_K = 128 elements (64 packed bytes)");
    DG_STATIC_ASSERT(BLOCK_K % UMMA_K == 0, "BLOCK_K must be a multiple of UMMA_K");
    DG_STATIC_ASSERT(kSwizzleAMode == 64 and kSwizzleBMode == 64, "Packed FP4 with BLOCK_K=128 requires 64B swizzle");
    DG_STATIC_ASSERT(kMajorA == cute::UMMA::Major::K and kMajorB == cute::UMMA::Major::K,
                     "Packed FP4 kernel currently supports K-major operands only");
    ...
    // SF configs: with VS=32 and BLOCK_K=256, BLOCK_K consumes 8 SFs per row.
    // kNumSFAStagesPerLoad=1 keeps the original load cadence (one TMA per K block).
    ...
    DG_STATIC_ASSERT(kGranKA == 32 and kGranKB == 32, "Packed FP4 path requires VS=32 (gran_k = 32)");
    ...
    // Shared memory sizes (packed FP4: BLOCK_K/2 bytes per element-row)
    ...
    // With BLOCK_K=128 packed FP4 (=64 packed bytes per row) and SWIZZLE_64B,
    // the natural A/B SMEM alignment is 512 bytes (one swizzle atom). Tighter
    // alignment is not required by the TMA/UMMA hardware.
    DG_STATIC_ASSERT(SMEM_CD_SIZE % 1024 == 0 and SMEM_A_SIZE_PER_STAGE % 512 == 0 and SMEM_B_SIZE_PER_STAGE % 512 == 0,
                     "Shared memory of A/B must be aligned to 512 bytes (one SWIZZLE_64B atom)");
    constexpr uint32_t UMMA_A_SIZE_PER_STAGE = math::constexpr_align(LOAD_BLOCK_M, LAYOUT_AD_M) * BLOCK_K_BYTES;
    ...
    // SMEM layout: align to 1024 bytes
    ...
    // A and B SMEM are byte-addressed (packed FP4). Keep raw byte pointers.
    ...
    auto smem_b_bytes = utils::PatternVisitor([&](const uint32_t& i) {
        return smem_buffer + SMEM_CD_SIZE + kNumStages * SMEM_A_SIZE_PER_STAGE + i * SMEM_B_SIZE_PER_STAGE;
    });
    ...
    // SMEM-descriptor base byte stride between MN atoms (SWIZZLE_64B atom = 64 bytes wide).
    // For K-major + SWIZZLE_64B layout, SBO = num_non_contiguous * BLOCK_K_BYTES
    // with num_non_contiguous = 128 / 16 = 8.
    ...
    if (warp_idx == 0 and cute::elect_one_sync()) {
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            for (uint32_t k_block_idx = 0; k_block_idx < num_total_k_blocks; advance_pipeline(k_block_idx)) {
                ...
                // Packed FP4 TMA: each row is one swizzle atom (128B), so a single
                // SM90_TMA_LOAD_2D per operand issues the full LOAD_BLOCK_{M,N} x
                // BLOCK_K box. Every CTA in the cluster does this independently.
                packed_fp4_tma_load_2d(&tensor_map_a, full_barriers[stage_idx], smem_a_bytes[stage_idx], k_a_idx, m_idx);
                packed_fp4_tma_load_2d(&tensor_map_b, full_barriers[stage_idx], smem_b_bytes[stage_idx], k_b_idx, n_idx);
        ...
        // Build base SMEM descriptors directly from packed byte pointers. The layout
        // is SWIZZLE_64B with K-major operand: SBO = 8 * BLOCK_K_BYTES, LBO = 0.
        auto a_desc = mma::sm100::make_smem_desc(cute::UMMA::LayoutType::SWIZZLE_64B,
                                                 smem_a_bytes[0], kSmemDescStrideBytesA, 0u);
        auto b_desc = mma::sm100::make_smem_desc(cute::UMMA::LayoutType::SWIZZLE_64B,
                                                 smem_b_bytes[0], kSmemDescStrideBytesB, 0u);
        // Per-stage low-half adjustment (descriptor stores byte_addr >> 4).
            ...
            for (uint32_t k_block_idx = 0; k_block_idx < num_total_k_blocks; advance_pipeline(k_block_idx)) {
                ...
                if (cute::elect_one_sync()) {
                    ...
                    using mma_t = cute::conditional_t<
                        kNumMulticast == 1, ptx::SM100_MMA_MXFP4_SS, ptx::SM100_MMA_MXFP4_2x1SM_SS>;
                    #pragma unroll
                    for (uint32_t k = 0; k < BLOCK_K / UMMA_K; ++ k) {
                        ...
                        // Per-iteration K byte offset = k * UMMA_K / 2 (packed FP4).
                        const uint32_t k_byte_offset = (k * UMMA_K) >> 1;
