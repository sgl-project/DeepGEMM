...
template <cute::UMMA::Major kMajorA, cute::UMMA::Major kMajorB,
          ...
          typename epilogue_op_t, bool kUsePaddedSFLayout = true>
...
sm100_fp8_fp4_gemm_1d1d_impl(int* grouped_layout,
    ...
    DG_STATIC_ASSERT(not is_k_grouped_contiguous(kGemmType) or kKAlignment % UMMA_K == 0,
                     "K alignment must be divisible by UMMA K");
    ...
    auto scheduler = sched::Scheduler<kGemmType, BLOCK_M, BLOCK_N, kNumGroups, kNumMulticast, kIsMulticastOnA, kNumSMs, kEnsureZeroPadding, kKAlignment, kGranKA * 4,
        sched::get_num_1d_blocks_per_group<kGemmType, BLOCK_M, BLOCK_N, kNumSMs, kIsMulticastOnA>(), 1, kUsePaddedSFLayout, kWithAccumulation>(
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            for (uint32_t k_block_idx = 0; k_block_idx < num_total_k_blocks; advance_pipeline(k_block_idx)) {
                ...
                if (cute::elect_one_sync()) {
                    ...
                    auto issue_umma = [&]<uint32_t umma_k_idx>() {
                    ...
                    if constexpr (is_k_grouped_contiguous(kGemmType) and kKAlignment % BLOCK_K != 0) {
                        // TMA may include the next group; consume only this group's UMMA-sized tail.
                        // Empty groups still initialize the accumulator for the zero-output epilogue.
                        const auto tail_k = scheduler.current_shape_k % BLOCK_K;
                        if (k_block_idx == num_total_k_blocks - 1 and tail_k != 0) {
                            utils::for_each_static_prefix(std::make_integer_sequence<uint32_t, BLOCK_K / UMMA_K>(),
                                                          math::ceil_div(tail_k, UMMA_K), issue_umma);
                        } else {
                            issue_full_k_block();
                        }
                    } else {
