// F5: Packed-FP4 x packed-FP4 MoE kernel using tcgen05.mma.kind::mxf4.
//
// Clone of sm100_fp8_fp4_mega_moe.cuh with the F4 packed-FP4 modifications
// (validated by sm100_fp4_fp4_gemm_1d1d.cuh on a standalone GEMM):
//   1. Both A (acts) and B (weights) operands use packed `cutlass::float_e2m1_t`
//      (2 elements per byte) in SMEM and in the global pool buffers.
//   2. UMMA_K doubles 32 -> 64 to match mxf4 block_scale.block32.
//   3. A/B SMEM is halved (BLOCK_K/2 bytes per row).
//   4. The MMA wrapper switches to ptx::SM100_MMA_MXFP4_2x1SM_SS, emitting
//      tcgen05.mma.cta_group::2.kind::mxf4.block_scale.
//   5. SMEM descriptors are built manually with packed-byte strides (the
//      generic cute helper assumes sizeof(dtype_t)=1 and would over-count K).
//   6. A/B TMA loads go through a packed-FP4 2-CTA helper so the per-row
//      swizzle atom matches the packed-FP4 tensor map (smem_inner_dim =
//      swizzle_mode * 2 packed elements).
//   7. The L1 epilogue casts SwiGLU output to FP4 (e2m1) instead of FP8.
//
// R1 (env DG_BLOCK_K): two BLOCK_K points share one code path. BLOCK_K=128
// (default) is the original path: 64B swizzle, one SF uint32 column, one
// UMMA_BLOCK_K sub-chunk per k-block. BLOCK_K=256 makes a packed row 128B,
// enabling 128B swizzle and halving the K-iteration (and barrier) count; the
// per-stage K span is then two SF columns and two UMMA_BLOCK_K sub-chunks,
// iterated by the two-level umma_k loop in the MMA warp. With DG_BLOCK_K unset
// every formula degenerates to the 128 values, so the generated kernel is
// byte-identical to the pre-R1 baseline.
//
// Pool/symbuffer assumption: when this kernel is selected, the host runtime
// allocates the L1/L2 activation pools at *FP4* element width (half the
// FP8 byte count). Dispatch warps therefore TMA-copy half as many bytes per
// token. Inputs into the kernel must already be packed FP4 (the test path
// or caller must pre-quantize BF16/FP8 -> FP4 before invoking the kernel).
//
// Env gating: this kernel is selected via DG_MEGA_MOE_FP4=1 at the
// runtime-dispatch site; the FP8 MoE kernel and its tunings are otherwise
// unchanged.
...
// Helper: 2D 2-CTA TMA load for packed-FP4 operands. Each row is one swizzle
// atom (swizzle_mode * 2 packed elements), so we issue
// `BLOCK_K_BYTES * 2 / (kSwizzleBytes * 2)` 2-CTA TMA loads. We pass *byte*
// offsets so the caller doesn't need to know about element vs. byte widths.
//
// NOTE (history): F7 originally hit an mbarrier `complete_tx::bytes` mismatch
// with this instruction (packed-FP4, 64B swizzle, 128-row outer dim) when the
// expected-tx accounting assumed FP8-kernel-style 1x byte delivery; F8 worked
// around it with per-CTA 1-CTA TMA (`packed_fp4_1sm_tma_load_2d`, now unused).
// F18 restored this 2-CTA path for BOTH A and B with the correct accounting:
// each CTA issues its own `cta_group::2` load, and ALL transaction bytes are
// signaled to the pair leader's barrier (peer bit masked in the cute copy), so
// the leader expects 2x the per-CTA stage bytes. This is the active path.
template <uint32_t BLOCK_K_BYTES, uint32_t LOAD_BLOCK_MN, uint32_t kSwizzleBytes>
CUTLASS_DEVICE void packed_fp4_2sm_tma_load_2d(
    void const* desc_ptr,
    cutlass::arch::ClusterTransactionBarrier* barrier_ptr,
    uint8_t* smem_ptr_bytes,
    const uint32_t& inner_elem_idx, const uint32_t& outer_idx) {
#if (defined(__CUDA_ARCH__) and (__CUDA_ARCH__ >= 1000))
    constexpr uint32_t kInnerAtomPackedElems = kSwizzleBytes * 2;
    constexpr uint32_t kBlockKPackedElems = BLOCK_K_BYTES * 2;
    constexpr uint32_t kNumAtoms = kBlockKPackedElems / kInnerAtomPackedElems;
    constexpr uint32_t kAtomBytes = (kInnerAtomPackedElems / 2) * LOAD_BLOCK_MN;
    #pragma unroll
    for (uint32_t i = 0; i < kNumAtoms; ++ i) {
        cute::SM100_TMA_2SM_LOAD_2D::copy(
            desc_ptr, reinterpret_cast<uint64_t*>(barrier_ptr),
            static_cast<uint64_t>(cute::TMA::CacheHintSm100::EVICT_NORMAL),
            smem_ptr_bytes + i * kAtomBytes,
            inner_elem_idx + i * kInnerAtomPackedElems, outer_idx);
    }
#endif
}

// Helper: 2D 1-CTA TMA load for packed-FP4 operands (F4-style).
//
// UNUSED since F18 (kept as the known-good fallback for the 2-CTA path above).
// Each CTA in the cluster issues its OWN 1-CTA `SM90_TMA_LOAD_2D` for its
// local slice of A (or its replicated copy of B), targeting the LOCAL
// transaction mbarrier. The 2-CTA UMMA descriptor then reads A from across
// the cluster's SMEM via cluster-shared addressing.
//
// We mirror `packed_fp4_2sm_tma_load_2d`'s atom decomposition so the
// per-row swizzle atom matches the packed-FP4 tensor map.
template <uint32_t BLOCK_K_BYTES, uint32_t LOAD_BLOCK_MN, uint32_t kSwizzleBytes>
CUTLASS_DEVICE void packed_fp4_1sm_tma_load_2d(
    void const* desc_ptr,
    cutlass::arch::ClusterTransactionBarrier* barrier_ptr,
    uint8_t* smem_ptr_bytes,
    const uint32_t& inner_elem_idx, const uint32_t& outer_idx) {
...
sm100_fp4_mega_moe_impl(void* y,
    ...
    // Token and buffer layouts (FP4 path: hidden activations stored as packed FP4,
    // i.e. kHidden / 2 bytes per token).
    ...
    constexpr auto fp4_sf_layout = layout::Data(kHidden / 32);
    constexpr auto fp4_intermediate_sf_layout = layout::Data(kIntermediateHidden / 32);
    ...
    const auto input_sf_buffer = layout::Buffer(
        fp4_sf_layout, 1, kNumMaxTokensPerRank,
    ...
    const auto l1_sf_buffer = layout::Buffer(
        fp4_sf_layout, 1, kNumPaddedSFPoolTokens,
    ...
    const auto l2_sf_buffer = layout::Buffer(
        fp4_intermediate_sf_layout, 1, kNumPaddedSFPoolTokens,
    ...
    // Data types
    // NOTES: both activations and weights are packed FP4 (e2m1, 2 elems/byte)
    using a_dtype_t = cutlass::float_e2m1_t;
    using b_dtype_t = cutlass::float_e2m1_t;

    // MMA configs (mxf4: UMMA_K doubles from 32 to 64 elements per instruction)
    ...
    // R1: two BLOCK_K points. 128 -> 64B-swizzle, one SF column, one UMMA_BLOCK_K
    // sub-chunk (today's path). 256 -> 128B-swizzle, two SF columns, two
    // UMMA_BLOCK_K sub-chunks. UMMA_BLOCK_K is the K-element span of one SF
    // uint32 column (4 VS=32 sub-scales = 128 elements).
    DG_STATIC_ASSERT(BLOCK_K == 128 or BLOCK_K == 256, "Invalid block K");
    DG_STATIC_ASSERT(BLOCK_K % UMMA_K == 0, "BLOCK_K must be a multiple of UMMA_K");
    constexpr uint32_t UMMA_BLOCK_K = 128;
    DG_STATIC_ASSERT(BLOCK_K % UMMA_BLOCK_K == 0, "BLOCK_K must be a multiple of UMMA_BLOCK_K");

    // Per-row byte width for packed FP4 SMEM
    constexpr uint32_t BLOCK_K_BYTES = BLOCK_K / 2;

    // Swizzle configs: packed FP4's swizzle atom is one row (BLOCK_K_BYTES).
    // BLOCK_K=128 -> 64B/row -> 64B swizzle; BLOCK_K=256 -> 128B/row -> 128B
    // swizzle. In packed_fp4_2sm_tma_load_2d, kNumAtoms stays 1 at both settings
    // (kInnerAtomPackedElems = kSwizzleBytes*2 = BLOCK_K elements), so a single
    // 2-CTA TMA issue per operand moves the whole row, now 2x bytes at 256.
    constexpr uint32_t kSwizzleAMode = (BLOCK_K_BYTES >= 128 ? 128 : 64);
    constexpr uint32_t kSwizzleBMode = (BLOCK_K_BYTES >= 128 ? 128 : 64);
    ...
    // Shared memory sizes
    // NOTES: L1 CD output cast to FP4 (post-SwiGLU half-width N), BF16 output for L2.
    // L1 output is packed FP4, so the per-byte width is L1_OUT_BLOCK_N / 2.
    constexpr uint32_t L1_OUT_BLOCK_N = BLOCK_N / 2;          // in *elements*
    constexpr uint32_t L1_OUT_BLOCK_N_BYTES = L1_OUT_BLOCK_N / 2; // packed FP4
    ...
    // Packed FP4: BLOCK_K elements -> BLOCK_K/2 bytes
    ...
    // One SF uint32 column per UMMA_BLOCK_K (128) K-elements; BLOCK_K=256 -> 2.
    constexpr uint32_t SMEM_SFA_SIZE_PER_STAGE = SF_BLOCK_M * sizeof(uint32_t) * (BLOCK_K / UMMA_BLOCK_K);
    constexpr uint32_t SMEM_SFB_SIZE_PER_STAGE = SF_BLOCK_N * sizeof(uint32_t) * (BLOCK_K / UMMA_BLOCK_K);
    constexpr uint32_t SMEM_CD_L1_SIZE =
        kNumEpilogueWarpgroups * STORE_BLOCK_M * L1_OUT_BLOCK_N_BYTES * kNumTMAStoreStages;
    ...
    // For packed FP4 the natural A/B SMEM atom is `kSwizzleMode * 8` bytes (one
    // swizzle atom across 8 non-contiguous rows): 512 at 64B swizzle, 1024 at
    // 128B. The min block_m=32 case gives A = 16 rows * 128B = 2048 (128B) ✓.
    DG_STATIC_ASSERT(SMEM_CD_SIZE % kSharedMemoryAlignment == 0 and
                     SMEM_A_SIZE_PER_STAGE % (kSwizzleAMode * 8) == 0 and
                     SMEM_B_SIZE_PER_STAGE % (kSwizzleBMode * 8) == 0,
                     "Shared memory of CD/A/B must be aligned to the swizzle atom");
    ...
    // D/A/B shared memory.
    // For packed FP4, A/B SMEM is byte-addressed (sizeof(float_e2m1_t)==1 in CUTLASS
    // doesn't reflect the packed 4-bit storage, so we keep raw byte pointers).
    auto smem_cd = utils::PatternVisitor([=](const uint32_t& i) {
        return math::advance_ptr<uint8_t>(smem_gemm_base, i * SMEM_CD_L1_SIZE_PER_STAGE);
    });
    auto smem_cd_l2 = smem_cd[0];
    auto smem_a_bytes = utils::PatternVisitor([=](const uint32_t& i) {
        return math::advance_ptr<uint8_t>(smem_gemm_base, SMEM_CD_SIZE + i * SMEM_A_SIZE_PER_STAGE);
    });
    auto smem_b_bytes = utils::PatternVisitor([=](const uint32_t& i) {
        return math::advance_ptr<uint8_t>(smem_gemm_base, SMEM_CD_SIZE + kNumStages * SMEM_A_SIZE_PER_STAGE + i * SMEM_B_SIZE_PER_STAGE);
    });
    ...
    if (warp_idx == 0) {
        ...
        if (cute::elect_one_sync()) {
            ...
            for (uint32_t i = 0; i < kNumStages; ++ i) {
                // F18: 2-CTA cooperative TMA barriers (init=4): leader CTA's
                // A-loader + B-loader arrive_and_expect_tx, non-leader's A-loader
                // + B-loader arrive(0). Matches FP8 kernel's 2-CTA pattern.
    ...
    if (warp_idx < kNumDispatchWarps) {
        ...
        for (uint32_t token_idx = sm_idx * kNumDispatchWarps + warp_idx; ; token_idx += kNumGlobalWarps) {
            ...
            // FP4 path: token bytes = kHidden / 2 (packed FP4).
            constexpr uint32_t kHiddenBytesFP4 = kHidden / 2;
            if (cute::elect_one_sync()) {
                ptx::tma_load_1d(
            ...
            if (cute::elect_one_sync()) {
                ...
                *l1_topk_weights_buffer.get_data_buffer(pool_token_idx).get_base_ptr<float>() = weight;
                ...
                // Wait for TMA token load to complete (packed FP4: kHidden/2 bytes)
                ptx::mbarrier_arrive_and_set_tx(pull_mbarrier, kHiddenBytesFP4);
        ...
        scheduler.for_each_block([&](const sched::BlockPhase& block_phase,
            ...
            if (block_phase == sched::BlockPhase::Linear1) {
                ...
                // R1: BLOCK_K-independent constexpr mask (ported from the FP8
                // kernel). The producers set one bit per L1 N-block (= L2 K-block);
                // the L2 K-iteration count num_k_blocks halves at BLOCK_K=256, so a
                // num_k_blocks-derived mask would silently wait on too few bits.
                // kShiftOffset counts all L1 N-blocks (= L2_SHAPE_K / BLOCK_N * 2).
            ...
            for (uint32_t k_block_idx = 0; k_block_idx < num_k_blocks; advance_pipeline(k_block_idx)) {
                ...
                uint32_t sfa_k_idx = k_block_idx * (BLOCK_K / UMMA_BLOCK_K);

                // Add 2 CTA offsets for non-leader CTA
                if (not is_leader_cta)
                    m_idx += scheduler.template get_valid_m<true>() / 2;

                // F18: 2-CTA cooperative TMA for A (matching FP8 pattern).
                // Both CTAs issue packed_fp4_2sm_tma_load_2d with their own
                // m_idx (CTA0 loads lower half, CTA1 loads upper half).
                // The leader CTA's barrier tracks all TX bytes.
                if (cute::elect_one_sync()) {
                    packed_fp4_2sm_tma_load_2d<BLOCK_K_BYTES, LOAD_BLOCK_M, kSwizzleAMode>(
                        tensor_map_a_ptr, full_barriers[stage_idx], smem_a_bytes[stage_idx], k_idx, m_idx);
                    // sf_box_outer_dim on the descriptor delivers BLOCK_K/UMMA_BLOCK_K
                    // SF columns in one copy, landing at stride SF_BLOCK_M words.
        ...
        scheduler.for_each_block([&](const sched::BlockPhase& block_phase,
            ...
            for (uint32_t k_block_idx = 0; k_block_idx < num_k_blocks; advance_pipeline(k_block_idx)) {
                ...
                uint32_t sfb_k_idx = local_expert_idx * shape_sfb_k + k_block_idx * (BLOCK_K / UMMA_BLOCK_K);

                // F18: 2-CTA cooperative TMA for B (matching FP8 pattern).
                // Each CTA loads its own full copy of the B tile (identical
                // coords); the leader's barrier expects 2x SMEM_B bytes. Wire
                // bytes equal the FP8 kernel's unpacked 1x broadcast (packed
                // rows are half-size), and the duplicate fetch is L2-served,
                // so HBM B traffic is not doubled.
                if (cute::elect_one_sync()) {
                    packed_fp4_2sm_tma_load_2d<BLOCK_K_BYTES, LOAD_BLOCK_N, kSwizzleBMode>(
                        tensor_map_b_ptr, full_barriers[stage_idx], smem_b_bytes[stage_idx], k_idx, n_idx);
                    // sf_box_outer_dim on the descriptor delivers BLOCK_K/UMMA_BLOCK_K
                    // SF columns in one copy, landing at stride SF_BLOCK_N words.
                    tma::copy<BLOCK_N, 1, 0>(
                        tensor_map_sfb_ptr, full_barriers[stage_idx], smem_sfb[stage_idx], sfb_n_idx, sfb_k_idx, 2);
                    if (is_leader_cta) {
                        full_barriers[stage_idx]->arrive_and_expect_tx(
                            SMEM_B_SIZE_PER_STAGE * 2 + SMEM_SFB_SIZE_PER_STAGE * 2);
                    } else {
                        full_barriers[stage_idx]->arrive(0u);
                    }
                }
        ...
        if (is_leader_cta) {
            ...
            // Packed FP4: build SMEM descriptors directly from byte pointers. The
            // SBO = num_non_contiguous * BLOCK_K_BYTES with num_non_contiguous =
            // 128 / 16 = 8 (512 at 64B swizzle, 1024 at 128B). The layout atom is
            // 64B-swizzle at BLOCK_K=128, 128B-swizzle at BLOCK_K=256.
            constexpr auto kLayoutTypeA = kSwizzleAMode == 128 ?
                cute::UMMA::LayoutType::SWIZZLE_128B : cute::UMMA::LayoutType::SWIZZLE_64B;
            constexpr auto kLayoutTypeB = kSwizzleBMode == 128 ?
                cute::UMMA::LayoutType::SWIZZLE_128B : cute::UMMA::LayoutType::SWIZZLE_64B;
            constexpr uint32_t kSmemDescStrideBytesA = 8u * BLOCK_K_BYTES;
            constexpr uint32_t kSmemDescStrideBytesB = 8u * BLOCK_K_BYTES;
            auto a_desc = mma::sm100::make_smem_desc(kLayoutTypeA,
                                                     smem_a_bytes[0], kSmemDescStrideBytesA, 0u);
            auto b_desc = mma::sm100::make_smem_desc(kLayoutTypeB,
                                                     smem_b_bytes[0], kSmemDescStrideBytesB, 0u);
            ...
            scheduler.for_each_block([&](const sched::BlockPhase& block_phase,
                ...
                for (uint32_t k_block_idx = 0; k_block_idx < num_k_blocks; advance_pipeline(k_block_idx)) {
                    ...
                    if (cute::elect_one_sync()) {
                        ...
                        // R1 two-level K loop: each `umma_k` covers UMMA_BLOCK_K=128
                        // K-elements = one SF uint32 column. At BLOCK_K=128 this runs
                        // once (today's path); at 256 it runs twice. Each umma_k's
                        // SF column is UTCCP'd into the SAME TMEM columns (reused),
                        // then its UMMA_BLOCK_K/UMMA_K=2 UMMAs are issued.
                        #pragma unroll
                        for (uint32_t umma_k = 0; umma_k < BLOCK_K / UMMA_BLOCK_K; ++ umma_k) {
                            // UTCCP copy this umma_k's SFA and SFB column to TMEM
                            #pragma unroll
                            for (uint32_t i = 0; i < SF_BLOCK_M / kNumUTCCPAlignedElems; ++ i) {
                                auto smem_ptr = smem_sfa[stage_idx] + umma_k * SF_BLOCK_M + i * kNumUTCCPAlignedElems;
                                mma::sm100::replace_smem_desc_addr(sf_desc, smem_ptr);
                                cute_utccp_t::copy(sf_desc, kTmemStartColOfSFA + i * 4);
                            }
                            #pragma unroll
                            for (uint32_t i = 0; i < SF_BLOCK_N / kNumUTCCPAlignedElems; ++ i) {
                                auto smem_ptr = smem_sfb[stage_idx] + umma_k * SF_BLOCK_N + i * kNumUTCCPAlignedElems;
                                mma::sm100::replace_smem_desc_addr(sf_desc, smem_ptr);
                                cute_utccp_t::copy(sf_desc, kTmemStartColOfSFB + i * 4);
                            }

                            // Issue UMMA (mxf4 path):
                            // - mxf4 consumes 2 SFs per UMMA_K=64 (VS=32 -> 64/32=2 SFs),
                            //   so sf_id = k*2 within this umma_k's column.
                            // - Packed-FP4 K byte offset into the row = K-elems >> 1.
                            #pragma unroll
                            for (uint32_t k = 0; k < UMMA_BLOCK_K / UMMA_K; ++ k) {
                                const uint32_t sfa_id = k * 2;
                                const uint32_t sfb_id = k * 2;
                                const auto runtime_instr_desc =
                                    mma::sm100::make_runtime_instr_desc_with_sf_id(instr_desc, sfb_id, sfa_id);
                                const uint32_t k_byte_offset = (umma_k * UMMA_BLOCK_K + k * UMMA_K) >> 1;
                                a_desc.lo = a_desc_base_lo + (k_byte_offset >> 4u);
                                b_desc.lo = b_desc_base_lo + (k_byte_offset >> 4u);
                                ptx::SM100_MMA_MXFP4_2x1SM_SS::fma(
                                    b_desc, a_desc, accum_stage_idx * UMMA_N,
                                    k_block_idx > 0 or umma_k > 0 or k > 0, runtime_instr_desc,
        ...
        scheduler.for_each_block([&](const sched::BlockPhase& block_phase,
            ...
            if (block_phase == sched::BlockPhase::Linear1) {
                ...
                for (uint32_t s = 0; s < WG_BLOCK_M / STORE_BLOCK_M; ++ s) {
                    ...
                    // F20: Fused atom loop — compute SwiGLU, amax, cross-warp reduce,
                    // shuffle, FP4-pack, and store all within one atom iteration.
                    // This eliminates the swiglu_values[kNumAtomsPerStore*2] and
                    // amax_values[kNumAtomsPerStore] arrays that kept 24+ floats live
                    // across the barrier between the old split loops, reducing register
                    // pressure.
                    const uint32_t tma_stage_idx = s % kNumTMAStoreStages;

                    // Wait for previous TMA store completion before writing SMEM
                    ptx::tma_store_wait<kNumTMAStoreStages - 1>();

                    constexpr uint32_t kFP4BankGroupBytes = kNumBankGroupBytes / 2;
                    #pragma unroll
                    for (uint32_t i = 0; i < kNumAtomsPerStore; ++ i) {
                        ...
                        // Amax reduction (within-warp, 4-lane reduce)
                        float2 amax_val;
                        amax_val.x = math::warp_reduce<4, true>(
                            cute::max(cute::abs(swiglu_vals[0].x), cute::abs(swiglu_vals[1].x)),
                            math::ReduceMax<float>());
                        amax_val.y = math::warp_reduce<4, true>(
                            cute::max(cute::abs(swiglu_vals[0].y), cute::abs(swiglu_vals[1].y)),
                            math::ReduceMax<float>());

                        // Store partial amax to SMEM for cross-warp exchange
                        if (lane_idx < 4)
                            smem_amax_reduction[epilogue_warp_idx * (STORE_BLOCK_M / 2) + i * (ATOM_M / 2) + lane_idx] = amax_val;

                        // Cross-warp barrier to make amax visible to partner warp
                        ptx::sync_aligned(128, kEpilogueWGBarrierStartIdx + epilogue_wg_idx);

                        // Read partner warp's amax and compute final amax
                        const float2 wp_amax =
                            smem_amax_reduction[(epilogue_warp_idx ^ 1) * (STORE_BLOCK_M / 2) + i * (ATOM_M / 2) + lane_idx % 4];
                        amax_val.x = cute::max(amax_val.x, wp_amax.x);
                        amax_val.y = cute::max(amax_val.y, wp_amax.y);

                        // Calculate SF for FP4 (e2m1 max = 6.0)
                        float2 sf, sf_inv;
                        math::get_e2m1_sf_and_sf_inv(amax_val, sf, sf_inv);

                        // Scale to FP4-range
                        const float2 upper = __fmul2_rn(swiglu_vals[0], sf_inv);
                        const float2 lower = __fmul2_rn(swiglu_vals[1], sf_inv);

                        // F13a: warp-shuffle reshuffle into (M=fixed, N=consecutive) layout.
                        float target_vals[4];
                        const uint32_t tgt_reg_set  = (lane_idx % 4) / 2;
                        const uint32_t tgt_component = (lane_idx / 4) % 2;
                        #pragma unroll
                        for (uint32_t k = 0; k < 4; ++ k) {
                            const uint32_t N = (lane_idx % 4) * 4 + k;
                            const uint32_t src_lane = (N % 8) * 4 + (lane_idx / 8);
                            const float v00 = __shfl_sync(0xFFFFFFFFu, upper.x, src_lane);
                            const float v01 = __shfl_sync(0xFFFFFFFFu, upper.y, src_lane);
                            const float v10 = __shfl_sync(0xFFFFFFFFu, lower.x, src_lane);
                            const float v11 = __shfl_sync(0xFFFFFFFFu, lower.y, src_lane);
                            const float v0 = (tgt_component == 0) ? v00 : v01;
                            const float v1 = (tgt_component == 0) ? v10 : v11;
                            target_vals[k] = (tgt_reg_set == 0) ? v0 : v1;
                        }

                        // Pack 4 FP4 values into 2 bytes (consecutive N at fixed M).
                        const auto fp4x2_lo = __nv_cvt_float2_to_fp4x2(
                            make_float2(target_vals[0], target_vals[1]), __NV_E2M1, cudaRoundNearest);
                        const auto fp4x2_hi = __nv_cvt_float2_to_fp4x2(
                            make_float2(target_vals[2], target_vals[3]), __NV_E2M1, cudaRoundNearest);
                        const uint16_t fp4x4_packed =
                            static_cast<uint16_t>(fp4x2_lo) | (static_cast<uint16_t>(fp4x2_hi) << 8);

                        // Store to SMEM with swizzle addressing
                        const uint32_t out_m = lane_idx / 4;
                        const uint32_t out_byte_col_lo = (lane_idx % 4) * 2;
                        const uint32_t warp_col = warp_idx_in_wg;
                        const uint32_t full_row = i * ATOM_M + out_m;
                        const uint32_t swizzle_bit = (full_row >> 2) & 1;
                        const uint32_t phys_warp_col = warp_col ^ (swizzle_bit << 1);
                        const auto smem_byte_ptr =
                            reinterpret_cast<uint8_t*>(smem_cd[tma_stage_idx])
                                + epilogue_wg_idx * STORE_BLOCK_M * L1_OUT_BLOCK_N_BYTES
                                + full_row * L1_OUT_BLOCK_N_BYTES
                                + phys_warp_col * kFP4BankGroupBytes
                                + out_byte_col_lo;
                        *reinterpret_cast<uint16_t*>(smem_byte_ptr) = fp4x4_packed;
                    ...
                    // FP4: SMEM byte stride is L1_OUT_BLOCK_N_BYTES, but the TMA
                    // descriptor still uses element coordinates for the N dim.
        ...
        // NOTES: 1, 2, or 4 chunks. Packed-FP4 A/B SMEM is halved versus FP8, so
        // we may need 4 chunks to fit the combine staging within SMEM_BEFORE_BARRIER_SIZE.
        // NOTES: Restrict on both smem and register
        constexpr uint32_t kNumChunks =
            (kNumChunkSlots * kNumEpilogueWarps * kNumHiddenBytes <= SMEM_BEFORE_BARRIER_SIZE and kHidden <= 32 * kNumMaxRegistersForBuffer)
                ? 1u
