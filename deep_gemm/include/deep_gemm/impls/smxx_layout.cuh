...
template <uint32_t kNumGroups, uint32_t kNumThreads,
          uint32_t BLOCK_MN, uint32_t BLOCK_PACKED_SF_K, bool kTransposed = true, bool kUsePsumLayout = false, bool kUsePaddedSFLayout = false>
CUTLASS_GLOBAL void pack_fp32_into_ue8m0(float* sf, uint32_t* out, uint32_t* grouped_layout,
    ...
    if constexpr (kNumGroups > 1 or kUsePsumLayout) {
        ...
        // FP32 inputs may contain logical rows (legacy) or physical padding rows.
        // Determine the source format from GPU metadata without a host synchronization.
        uint32_t compact_rows[4], padded_rows[4];
        uint32_t local_compact_rows = 0, local_padded_rows = 0;
        #pragma unroll
        for (uint32_t i = 0; i < 4; ++ i) {
            const auto group_idx = lane_idx * 4 + i;
            const auto prev_end = i == 0 ? __shfl_up_sync(0xffffffff, layout_cache[3], 1) : layout_cache[i - 1];
            const auto group_start = group_idx == 0 ? 0 : math::align(prev_end, k_alignment);
            const auto group_k = group_idx >= kNumGroups ? 0 :
                (kUsePsumLayout ? layout_cache[i] - group_start : layout_cache[i]);
            compact_rows[i] = math::ceil_div(group_k, gran_k);
            padded_rows[i] = math::ceil_div(math::align(group_k, k_alignment), gran_k);
            local_compact_rows += compact_rows[i];
            local_padded_rows += padded_rows[i];
        }
        const auto total_compact_rows = __reduce_add_sync(0xffffffff, local_compact_rows);
        const auto total_padded_rows = __reduce_add_sync(0xffffffff, local_padded_rows);
        const bool source_is_padded = kUsePsumLayout and sf_k != total_compact_rows;
        DG_TRAP_ONLY_DEVICE_ASSERT(sf_k == (source_is_padded ? total_padded_rows : total_compact_rows));
        ...
        for (uint32_t group_idx = 0; group_idx < kNumGroups; ++ group_idx) {
            const auto compact_group_rows = __shfl_sync(0xffffffff, compact_rows[group_idx % 4], group_idx / 4);
            const auto padded_group_rows = __shfl_sync(0xffffffff, padded_rows[group_idx % 4], group_idx / 4);
            const auto source_group_rows = source_is_padded ? padded_group_rows : compact_group_rows;
            const auto out_group_rows = kUsePaddedSFLayout ? padded_group_rows : compact_group_rows;
            const auto group_packed_rows = math::ceil_div(out_group_rows, 4u);
            if (packed_sf_k_idx < num_prefix_packed_rows + group_packed_rows) {
                group_sf_row_start = num_prefix_sf_rows;
                group_sf_row_end = group_sf_row_start + cute::min(source_group_rows, out_group_rows);
                num_padding_sf_rows = num_prefix_packed_rows * 4 - num_prefix_sf_rows;
                owner_group_found = true;
                break;
            }
            num_prefix_sf_rows += source_group_rows;
            num_prefix_packed_rows += group_packed_rows;
        }
