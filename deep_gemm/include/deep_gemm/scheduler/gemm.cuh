...
template <GemmType kGemmType,
          ...
          bool kUsePaddedSFLayout = true,
          bool kSkipEmptyKGroups = false>
struct Scheduler {
    // A/B group starts must be aligned to UMMA K steps. SF rows are packed
    // independently per group and tracked by `current_sf_k_cumsum`.
    DG_STATIC_ASSERT(not is_k_grouped_contiguous(kGemmType) or kKAlignment % 32 == 0,
                     "K alignment must be a multiple of 32");
    ...
    uint32_t current_num_valid_k_groups = 0;
    ...
    CUTLASS_DEVICE bool get_next_block(uint32_t& m_block_idx, uint32_t& n_block_idx) {
        ...
        if constexpr (kGemmType == GemmType::MGroupedMasked) {
            ...
            while (true) {
                ...
                // Within current group
                const auto scheduled_group_idx = kSkipEmptyKGroups ? current_num_valid_k_groups : current_group_idx;
                if ((not kSkipEmptyKGroups or current_shape_k > 0) and
                    next_block_idx < (scheduled_group_idx + 1) * num_blocks)
                    break;

                // Accumulation already preserved C in D; an empty group needs no kernel work.
                if constexpr (kSkipEmptyKGroups)
                    current_num_valid_k_groups += current_shape_k > 0;
                ...
                const auto aligned_shape_k = kUsePaddedSFLayout ? math::align(current_shape_k, kKAlignment) : current_shape_k;
                current_sf_k_cumsum += math::ceil_div(aligned_shape_k, kSFKSpan);
                get_next_k_group();
            }

            get_swizzled_block_idx(next_block_idx - (kSkipEmptyKGroups ? current_num_valid_k_groups : current_group_idx) * num_blocks, m_block_idx, n_block_idx);
        } else if constexpr (kGemmType == GemmType::Batched) {
