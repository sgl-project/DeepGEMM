...
def test_mega_mhc_api_contract() -> None:
    ...
    # The FFI boundary must preserve padded storage capacity and view offsets.
    inputs, _ = make_inputs(num_tokens, hidden, 2027, False)
    expected_outputs = make_outputs(inputs, 'extra')
    run_mega_mhc(inputs, expected_outputs)
    outputs = make_outputs(inputs, 'extra')
    storage = outputs['y_shared_sf_storage']
    guard_words, sentinel = 32, 0x1234567
    guarded = torch.full((storage.numel() + 2 * guard_words,), sentinel,
                         dtype=torch.int32, device='cuda')
    offset_storage = guarded[guard_words:-guard_words].view(storage.size(1), storage.size(0)).T
    outputs['y_shared_sf_storage'] = offset_storage
    outputs['y_shared_sf'] = offset_storage[:num_tokens]
    run_mega_mhc(inputs, outputs)
    expected = logical_outputs(expected_outputs)
    for name, tensor in logical_outputs(outputs).items():
        assert_bitwise_equal(tensor, expected[name], f'Offset SF storage: {name}')
    assert torch.all(guarded[:guard_words] == sentinel)
    assert torch.all(guarded[-guard_words:] == sentinel)

    # A logical strided view without the final padded rows remains invalid.
    outputs['y_shared_sf'] = torch.empty_strided(
        outputs['y_shared_sf'].shape, outputs['y_shared_sf'].stride(),
        dtype=torch.int32, device='cuda')
    try:
        run_mega_mhc(inputs, outputs)
    except RuntimeError as error:
        assert 'num_required_bytes' in str(error)
    else:
        raise AssertionError('Shared SF storage must include the padded rows')
...
@test_filter(lambda: get_arch_major() == 10)
@torch.no_grad()
def test_mega_mhc_graph_api_contract() -> None:
    num_tokens, hidden = 64, 4096
    inputs, _ = make_inputs(num_tokens, hidden, 2026, False)
    ...
    # A reused graph grid ID must not expose the preceding replay's terminal
    # split count. Alternate inputs so stale normalization cannot look correct.
    if not ref_mhc.has_baseline():
        raise RuntimeError("Mega mHC graph correctness requires the TileLang and tile-kernels reference operators")
    predecessor, _ = make_inputs(4096, 4096, 20269293, False)
    predecessor_outputs = make_outputs(predecessor, 'extra')
    run_mega_mhc(predecessor, predecessor_outputs)
    torch.cuda.synchronize()

    replay_inputs, duplicates = make_inputs(64, 7168, 20268334, True)
    alternate_inputs, _ = make_inputs(64, 7168, 20268335, True)
    replay_outputs = make_outputs(replay_inputs, 'col')
    references = []
    for state in (replay_inputs, alternate_inputs):
        kwargs = dict(**state, sf_layout='col', shared_sf_block_m=0)
        references.append(tuple(logical_outputs(reference(**kwargs)) for reference in
                                (ref_mhc.mhc_reference, ref_mhc.mhc_baseline)))
    input_states = [{name: tensor.clone() for name, tensor in state.items() if isinstance(tensor, torch.Tensor)}
                    for state in (replay_inputs, alternate_inputs)]
    cold_cache = torch.empty(512 * 1024 * 1024, dtype=torch.uint8, device='cuda')
    replay_stream = torch.cuda.Stream()
    with torch.cuda.stream(replay_stream):
        run_mega_mhc(replay_inputs, replay_outputs)
    replay_stream.synchronize()
    replay_graph = torch.cuda.CUDAGraph()
    with torch.cuda.graph(replay_graph, stream=replay_stream):
        run_mega_mhc(replay_inputs, replay_outputs)
    for batch in range(20):
        state_idx = batch % len(input_states)
        for name, tensor in input_states[state_idx].items():
            replay_inputs[name].copy_(tensor)
        # Queue replays without a host synchronization between launches; the
        # cache flush makes different CTA arrival orders observable.
        for _ in range(64):
            cold_cache.zero_()
            replay_graph.replay()
        torch.cuda.synchronize()
        actual = logical_outputs(replay_outputs)
        check_correctness(actual, *references[state_idx], ('shifted', 'col', 64, 7168, 'graph', batch))
        for token in duplicates:
            for name, tensor in actual.items():
                assert_bitwise_equal(tensor[token], tensor[0], f'CUDA Graph replay {batch}, token {token}: {name}')
...
def test_mega_mhc() -> None:
    if not ref_mhc.has_baseline():
        raise RuntimeError("Mega mHC correctness requires the TileLang and tile-kernels reference operators")
...
if __name__ == '__main__':
    ...
    test_mega_mhc_graph_api_contract()
    test_mega_mhc()
