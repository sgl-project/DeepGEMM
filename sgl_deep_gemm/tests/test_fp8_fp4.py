...
import deep_gemm
from deep_gemm.utils import align, ceil_div, pack_ue8m0_to_int
    ...
    for dtype, quant_config, gemm in test_options:
        ...
        for num_groups, m, n, major_a, major_b, real_ks_cpu, _, _, gran_k, k_alignment, use_psum_layout, accumulate, out_dtype in \
            ...
            for test_real_ks_cpu in enumerate_k_grouped_contiguous_test_variants(real_ks_cpu):
                for compact_sf in ((False, True) if use_psum_layout and not is_fp4 else (False, )):
                    total_k, a, b, c, d, ref_d, grouped_layout, host_ks_cpu = generate_k_grouped_contiguous(
                        ...
                        accumulate=accumulate, out_dtype=out_dtype, compact_sf=compact_sf)
                    ...
                    # Retain legacy gran-128 prepacked SF as well as gran-32.
                    if arch_major == 10:
                        sf_ks = [ceil_div(k, gran_k) for k in
                                 (test_real_ks_cpu if compact_sf else host_ks_cpu)]
                        packed_a_groups = [
                        ...
                        ref_packed_a = torch.cat(packed_a_groups) if packed_a_groups else torch.empty(
                            (0, m), dtype=torch.int32, device=a[1].device)
                        packed_b_groups = [
                            pack_ue8m0_to_int(torch.nn.functional.pad(
                                group_sf.T, (0, align(sf_k, 4) - sf_k)).contiguous()).T
                            for group_sf, sf_k in zip(b[1].split(sf_ks), sf_ks) if sf_k > 0
                        ]
                        ref_packed_b = torch.cat(packed_b_groups) if packed_b_groups else torch.empty(
                            (0, n), dtype=torch.int32, device=b[1].device)
                        pack_kwargs = {} if compact_sf else dict(use_padded_sf_layout=True)
                        packed_layout_kwargs = {} if is_fp4 else pack_kwargs
                        for packed_host_ks_cpu in host_ks_options:
                            packed_a = (
                                a[0], deep_gemm.get_k_grouped_mn_major_tma_aligned_packed_ue8m0_tensor(
                                    a[1], grouped_layout, packed_host_ks_cpu, gran_k, k_alignment, use_psum_layout,
                                    **pack_kwargs))
                            packed_b = (
                                b[0], deep_gemm.get_k_grouped_mn_major_tma_aligned_packed_ue8m0_tensor(
                                    b[1], grouped_layout, packed_host_ks_cpu, gran_k, k_alignment, use_psum_layout,
                                    **pack_kwargs))
                            if compact_sf or not packed_host_ks_cpu:
                                assert torch.equal(packed_a[1][:ref_packed_a.size(0)], ref_packed_a)
                                assert torch.equal(packed_b[1][:ref_packed_b.size(0)], ref_packed_b)
                            else:
                                assert torch.equal(packed_a[1], ref_packed_a)
                                assert torch.equal(packed_b[1], ref_packed_b)

                            packed_d = initial_d.clone()
                            gemm(packed_a, packed_b, packed_d, packed_host_ks_cpu, grouped_layout, packed_d if accumulate else None,
                                 recipe=recipe, use_psum_layout=use_psum_layout, **packed_layout_kwargs)
                            if accumulate:
                                ...
                                assert_direct_output_matches_fp32_accumulation(
                                    packed_d,
                                    lambda output, accumulator: gemm(
                                        packed_a, packed_b, output, packed_host_ks_cpu, grouped_layout, accumulator,
                                        recipe=recipe, use_psum_layout=use_psum_layout, **packed_layout_kwargs),
                                    case_label)
