...
try:
    ...
    # Public TileKernels 1.0.0's topk_gate only returns indices. Its ungrouped
    # top2_sum_gate includes scoring, normalized weights and physical mapping.
    from tile_kernels.moe import top2_sum_gate
    moe_topk_gate = None

BASELINE_KERNELS = (
    'nvjet', 'splitKreduce', 'cutlass',
    'moe_topk_gate' if moe_topk_gate is not None else 'top2_sum_gate',
)
...
def bench_baseline_gate(inputs: dict[str, Any]) -> float:
    ...
    if moe_topk_gate is None:
        # Only the performance baseline uses this adapter; all scoring and
        # routing-control correctness tests retain their PyTorch reference.
        assert inputs['image_bias'] is None and inputs['image_token_mask'] is None, \
            'The public TileKernels performance baseline does not support image bias'
        baseline_bias = inputs['bias']
        if baseline_bias is None:
            baseline_bias = torch.zeros(
    ...
    def run_baseline():
        ...
        if moe_topk_gate is None:
            return top2_sum_gate(
                logits, baseline_bias, NUM_TOPK,
                num_topk_groups=1, num_groups=1,
                use_shared_as_routed=False, num_shared_experts=NUM_SHARED_EXPERTS,
                routed_scaling_factor=ROUTED_SCALING_FACTOR,
                ep_rank=0, num_ep_ranks=1, tp_rank=0, num_tp_ranks=1,
                scoring_func='sqrtsoftplus', mask=inputs['mask'],
                to_physical_map=inputs['to_physical_map'], logical_count=inputs['logical_count'],
    ...
    if moe_topk_gate is None:
        baseline_idx, baseline_weights = run_baseline()
        check_topk_weights(
            inputs, dict(topk_idx=baseline_idx, topk_weights=baseline_weights, unmapped_topk_idx=None),
            logits, 'sqrtsoftplus', inputs['mask'])
...
def check_output_lifetime() -> None:
    num_topk = 8
    inputs = make_inputs(8, 4096, 128, with_bias=False, with_physical_map=False)
    inputs['x'].zero_()
    inputs['weight'].zero_()
    for temporary_outputs in (True, False):
        if temporary_outputs:
            topk_idx, topk_weights = run_mega_gate(
                inputs, make_outputs(inputs, num_topk=num_topk), 'identity', num_topk=num_topk)
        else:
            outputs = make_outputs(inputs, num_topk=num_topk)
            topk_idx, topk_weights = run_mega_gate(inputs, outputs, 'identity', num_topk=num_topk)
            assert topk_idx.data_ptr() == outputs['topk_idx'].data_ptr()
            assert topk_weights.data_ptr() == outputs['topk_weights'].data_ptr()
            del outputs

        # Reuse freed allocations if the returned tensors lost their owners.
        overwrite_idx = torch.full((8, num_topk), -1, dtype=torch.int64, device='cuda')
        overwrite_weights = torch.full((8, num_topk), -1, dtype=torch.float32, device='cuda')
        expected = torch.arange(num_topk, dtype=torch.int64, device='cuda').expand(8, -1)
        assert torch.equal(topk_idx, expected)
        assert torch.equal(topk_weights, torch.zeros_like(topk_weights))
        assert topk_idx.data_ptr() != overwrite_idx.data_ptr()
        assert topk_weights.data_ptr() != overwrite_weights.data_ptr()
