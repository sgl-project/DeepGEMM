...
def _ref_quant_fp8(x: torch.Tensor, group_size: int):
    """Per-token grouped absmax FP8-e4m3 quant with fp32 scales.
...
def _alloc(padded_max: int, hidden: int, top_k: int, group_size: int, device):
...
def _reference(
    x, topk_idx, topk_weights, padded_max, group_size, routed_scaling_factor
):
    M, _ = x.shape
    buf_x, buf_x_sf, buf_idx, buf_w = _alloc(
        padded_max, x.shape[1], topk_idx.shape[1], group_size, x.device)
    if M > 0:
        qx, sf = _ref_quant_fp8(x, group_size)
        buf_x[:M].copy_(qx)
        buf_x_sf[:M].copy_(sf)
...
def _run_kernel(
    ...
    K = topk_idx.shape[1]
    buf_x, buf_x_sf, buf_idx, buf_w = _alloc(
        padded_max, H, K, group_size, x.device)
...
def _check_case(M, H, K, P, group_size, scale, seed):
    assert H % group_size == 0 and H % 8 == 0
    ...
    ref_x, ref_sf, ref_idx, ref_w = _reference(
        x, topk_idx, topk_weights, P, group_size, scale)
    out_x, out_sf, out_idx, out_w = _run_kernel(
        x, topk_idx, topk_weights, P, group_size, scale)
    ...
    if M > 0:
        ...
        def _dequant(bx, bsf):
            return (
                bx[:M].float().reshape(M, G, group_size)
                * bsf[:M].unsqueeze(-1)
            )
        ...
        torch.testing.assert_close(
            out_deq, x.float().reshape(M, G, group_size),
            rtol=0.25, atol=1e-4
        )
    ...
    print(
        f"  PASS  M={M} H={H} K={K} P={P} group={group_size} "
        f"alpha={scale} ({pad})"
    )


# (M, H, K, padded_max, group_size, routed_scaling_factor)
CASES = [
    (0, 2048, 8, 32, 128, 1.0),       # zero tokens -> all pad
    (1, 1024, 4, 8, 128, 2.5),
    (7, 2048, 8, 16, 32, 1.0),
    (7, 3584, 16, 16, 32, 2.5),
    (32, 4096, 8, 32, 128, 2.5),
    (128, 7168, 8, 256, 128, 2.5),
    (128, 7168, 8, 128, 32, 1.0),    # exact-fill, no pad rows
]
...
def main() -> int:
    ...
    for i, (M, H, K, P, group_size, s) in enumerate(CASES):
        _check_case(
            M, H, K, P, group_size, s,
            seed=(args.seed ^ (0xC0FFEE + i)))
    print("OK")
    return 0
