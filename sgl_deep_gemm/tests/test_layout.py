...
def test_k_grouped_sf_layout_kernels() -> None:
    ...
    for mn, ks_cpu, num_groups, gran_k, k_alignment in enumerate_k_grouped_sf_layout():
        ...
        packed_sf = get_k_grouped_mn_major_tma_aligned_packed_ue8m0_tensor(fp32_sf, grouped_layout, ks_cpu, gran_k, k_alignment, use_padded_sf_layout=True)
        legacy_packed_sf = get_k_grouped_mn_major_tma_aligned_packed_ue8m0_tensor(
            fp32_sf, grouped_layout, ks_cpu, gran_k, k_alignment)
        assert torch.equal(legacy_packed_sf, packed_sf)
        ...
        # Performance
        t = bench_kineto(lambda: get_k_grouped_mn_major_tma_aligned_packed_ue8m0_tensor(fp32_sf, grouped_layout, ks_cpu, gran_k, k_alignment, use_padded_sf_layout=True), 'pack_fp32_into_ue8m0')
...
def test_k_grouped_psum_sf_layout_kernels() -> None:
    ...
    for mn, real_ks_cpu, aligned_ks_cpu, psum_layout, num_groups, gran_k, k_alignment in enumerate_k_grouped_psum_sf_layout():
        for compact_sf in (False, True):
            pack_kwargs = {} if compact_sf else dict(use_padded_sf_layout=True)
            grouped_layout = torch.tensor(psum_layout, dtype=torch.int, device='cuda')

            sf_ks = [ceil_div(k if compact_sf else align(k, k_alignment), gran_k) for k in real_ks_cpu]
            packed_sf_ks = [ceil_div(sf_k, 4) for sf_k in sf_ks]
            ...
            # Exercise both legacy logical-K SF rows and upstream aligned-K SF rows.
            # Each output group independently pads its final packed row.
            packed_sf = get_k_grouped_mn_major_tma_aligned_packed_ue8m0_tensor(fp32_sf, grouped_layout, aligned_ks_cpu, gran_k, k_alignment, use_psum_layout=True, **pack_kwargs)
            if compact_sf:
                assert torch.equal(packed_sf[:sum(packed_sf_ks)], ref_packed_sf)
                exact_packed_sf = get_k_grouped_mn_major_tma_aligned_packed_ue8m0_tensor(
                    fp32_sf, grouped_layout, real_ks_cpu, gran_k, k_alignment, use_psum_layout=True)
            ...
            unsynced_packed_sf = get_k_grouped_mn_major_tma_aligned_packed_ue8m0_tensor(
                fp32_sf, grouped_layout, None, gran_k, k_alignment, use_psum_layout=True, **pack_kwargs)
            ...
            empty_ks_packed_sf = get_k_grouped_mn_major_tma_aligned_packed_ue8m0_tensor(
                fp32_sf, grouped_layout, [], gran_k, k_alignment, use_psum_layout=True, **pack_kwargs)
            assert torch.equal(empty_ks_packed_sf[:sum(packed_sf_ks)], ref_packed_sf)

            # Performance
            t = bench_kineto(lambda: get_k_grouped_mn_major_tma_aligned_packed_ue8m0_tensor(fp32_sf, grouped_layout, aligned_ks_cpu, gran_k, k_alignment, use_psum_layout=True, **pack_kwargs), 'pack_fp32_into_ue8m0')
            print(f' > Perf ({num_groups=:3}, {mn=:5}, sum_k={sum(real_ks_cpu):5}, gran_k={gran_k:3}, k_alignment={k_alignment:3}, compact_sf={int(compact_sf)}):'
                  f'{t * 1e6:4.0f} us | '
                  f'{count_bytes(fp32_sf, packed_sf, grouped_layout) / 1e9 / t:4.0f} GB/s')
    print()
