...
def enumerate_m_grouped_contiguous(dtype: torch.dtype) -> Generator:
    quant_config_list = QuantConfig.get_list_from_dtype(dtype)
    # Keep the downstream tiny-per-expert decode coverage.
...
def enumerate_k_grouped_contiguous(dtype: torch.dtype):
    if dtype == torch.bfloat16:
        sf_layout_list = [(128, 128)] if get_arch_major() == 9 else [(128, 128), (256, 256), (384, 384), (32, 32), (192, 192)]
        ...
        sf_layout_list = ([(128, 128)] if get_arch_major() == 9 else
                          [(32, 128), (128, 128), (32, 256), (128, 256), (32, 384), (128, 384),
                           (32, 32), (32, 160), (32, 224), (128, 160), (128, 224)])
...
def enumerate_k_grouped_contiguous_test_variants(real_ks_cpu: List[int], include_k32_tail: bool = False):
...
def enumerate_k_grouped_sf_layout():
    sf_layouts = ([(128, 128)] if get_arch_major() == 9 else
                  [(32, 128), (128, 128), (32, 256), (128, 256), (32, 384), (128, 384), (32, 32)])
    for mn in (4096, 7168):
        for num_groups, avg_k in ((16, 2048), (8, 4096), (72, 384), (128, 256)):
            for gran_k, k_alignment in sf_layouts:
                set_mk_alignment_for_contiguous_layout(k_alignment)
...
def enumerate_k_grouped_psum_sf_layout():
    ...
    # Keep the downstream compact-SF group-start alignments independently of
    # the new padded-SF matrix, including K tails and small expert groups.
    for mn in (4096, 7168):
        for num_groups, avg_k in ((16, 2048), (8, 4096), (72, 384), (128, 256)):
            for gran_k in (32, 128):
                for k_alignment in (32, 160, 192, 224):
                    if gran_k == 32 and k_alignment == 32:
                        continue  # Already emitted by the matrix above.
                    set_mk_alignment_for_contiguous_layout(k_alignment)
                    real_ks_cpu = [max(1, int(random.uniform(0.7, 1.3) * avg_k))
                                   for _ in range(num_groups)]
                    aligned_ks_cpu = [align(k, k_alignment) for k in real_ks_cpu]
                    psum_layout = build_psum_layout_from_ks(real_ks_cpu, k_alignment)
                    yield mn, real_ks_cpu, aligned_ks_cpu, psum_layout, num_groups, gran_k, k_alignment
...
def k_grouped_cast_fp8_fp4_with_major(x: torch.Tensor, ks_cpu: List[int], major: MajorTypeAB,
    ...
    # Cast each group independently. sf_ks_cpu selects logical-K (legacy) or
    # aligned-K (upstream) SF spans, each rounded to a complete quantization block.
    ...
    for k, sf_k, end in zip(ks_cpu, sf_ks_cpu, group_ends):
        if sf_k == 0:
            continue
        start = end - k
        x_group = torch.zeros((align(sf_k, gran_k), mn), dtype=x.dtype, device=x.device)
        x_group[:k] = x[start:end]
...
def generate_k_grouped_contiguous(num_groups: int, m: int, n: int, major_a: MajorTypeAB, major_b: MajorTypeAB, logical_ks_cpu: List[int],
    ...
    if is_fp4:
        ...
        assert k_alignment % 32 == 0
    assert not (is_fp4 and compact_sf)
    sf_ks_cpu = logical_ks_cpu if compact_sf and use_psum_layout else host_ks_cpu
    ...
    quantized_a = k_grouped_cast_fp8_fp4_with_major(
        ...
        logical_group_ends if use_psum_layout else None, sf_ks_cpu)
