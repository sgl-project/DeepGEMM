...
def test_bf16_repeatability() -> None:
    ...
    if get_arch_major() != 12:
        return
    previous = deep_gemm.get_deterministic_algorithms()
...
def test_k_grouped_gemm_contiguous() -> None:
    ...
    for num_groups, m, n, major_a, major_b, real_ks_cpu, _, _, _, alignment, use_psum_layout, accumulate, out_dtype in \
        ...
        for test_real_ks_cpu in enumerate_k_grouped_contiguous_test_variants(
                real_ks_cpu, include_k32_tail=get_arch_major() == 10 and alignment == 32):
